...
import { Conversation, ConversationType } from '../conversation/schemas/conversation.schema';
...
export class ParticipantService {
  ...
  async createParticipant(conversationId: ObjectId, userId: ObjectId, role: ParticipantRole) {
  ...
  async setRole(setRoleDto: SetRoleDto) {
    ...
    const participantSetter = await this.getParticipant(setRoleDto.setterUserId, setRoleDto.conversationId);
    ...
    await this.entityManager.updateOne(Participant, {
  ...
  async banParticipant(banParticipantDto: BanParticipantDto) {
    // User got banned
    const bannedParticipant = await this.getParticipant(banParticipantDto.bannedUser, banParticipantDto.conversationId);
    if (null === bannedParticipant) {
      throw new Error('Banned participant not found');
    }
    // User banned user got banned
    const userBaner = await this.getParticipant(banParticipantDto.bannedBy, banParticipantDto.conversationId);
    if (null === userBaner) {
  ...
  async joinConversation(userId: ObjectId, conversationId: ObjectId) {
    ...
    // Check if participant exists
    const participant = await this.getParticipant(userId, conversationId);
    if (participant) throw new Error('You are already a participant of this conversation');

    // Create participant
    await this.createParticipant(conversationId, userId, ParticipantRole.MEMBER);
